...
function bufferToBase64urlIfNeeded(value) {
  ...
  if (ArrayBuffer.isView(value)) return base64urlEncode(Buffer.from(value.buffer))
  return value
}
...
function normalizeOptionsForJson(options) {
  ...
  try {
    out.challenge = bufferToBase64urlIfNeeded(options.challenge) || out.challenge
  } catch (_) {}
...
app.post('/webauthn/register/options', (req, res) => {
  ...
  if (!users.has(username)) {
    users.set(username, { id: base64urlEncode(crypto.randomBytes(32)), credentials: [], currentChallenge: undefined })
  }
  ...
  const options = generateRegistrationOptions({
    ...
    excludeCredentials: user.credentials.map((c) => ({ id: base64urlToBuffer(c.credentialID), type: 'public-key' })),
  })
  ...
  normalized.user.displayName = options.user?.displayName || displayName || username
  ...
  try {
    // Normalize credential payload from client (accept base64url strings)
      ...
      if (normalizedCred.response) {
        if (normalizedCred.response.attestationObject && !(typeof normalizedCred.response.attestationObject === 'string')) {
          normalizedCred.response.attestationObject = base64urlEncode(Buffer.from(normalizedCred.response.attestationObject))
        }
        if (normalizedCred.response.clientDataJSON && !(typeof normalizedCred.response.clientDataJSON === 'string')) {
          normalizedCred.response.clientDataJSON = base64urlEncode(Buffer.from(normalizedCred.response.clientDataJSON))
        }
      }
    ...
    if (verified && registrationInfo) {
      ...
      user.credentials.push({ credentialID: credIDb64, credentialPublicKey: base64urlEncode(credentialPublicKey), counter })
...
app.post('/webauthn/authn/options', (req, res) => {
  ...
  const allowCredentials = user.credentials.map((c) => ({ id: base64urlToBuffer(c.credentialID), type: 'public-key' }))
...
app.post('/webauthn/authn/verify', async (req, res) => {
  ...
  try {
    if (normalizedCred.rawId && typeof normalizedCred.rawId === 'string') {
      ...
      const buf = Buffer.from(normalizedCred.rawId.replace(/-/g, '+').replace(/_/g, '/'), 'base64')
      normalizedCred.rawId = base64urlEncode(buf)
    }
  } catch (e) {
    // ignore and leave as-is
  }
  if (!normalizedCred.id && normalizedCred.rawId) normalizedCred.id = normalizedCred.rawId

  if (normalizedCred.response) {
    if (normalizedCred.response.authenticatorData && !(typeof normalizedCred.response.authenticatorData === 'string')) {
      normalizedCred.response.authenticatorData = base64urlEncode(Buffer.from(normalizedCred.response.authenticatorData))
    }
    if (normalizedCred.response.clientDataJSON && !(typeof normalizedCred.response.clientDataJSON === 'string')) {
      normalizedCred.response.clientDataJSON = base64urlEncode(Buffer.from(normalizedCred.response.clientDataJSON))
    }
    if (normalizedCred.response.signature && !(typeof normalizedCred.response.signature === 'string')) {
      normalizedCred.response.signature = base64urlEncode(Buffer.from(normalizedCred.response.signature))
    }
    if (normalizedCred.response.userHandle && !(typeof normalizedCred.response.userHandle === 'string')) {
      try { normalizedCred.response.userHandle = base64urlEncode(Buffer.from(normalizedCred.response.userHandle)) } catch (_) {}
    }
  }

  // Find stored authenticator by matching normalized base64url id
  const stored = user.credentials.find((c) => c.credentialID === normalizedCred.id || c.credentialID === normalizedCred.rawId)
  if (!stored) return res.status(404).json({ error: 'authenticator not found' })
...
app.listen(PORT, () => console.log(`WebAuthn demo server listening on http://localhost:${PORT}
  RP ID: ${RP_ID}
  ORIGIN: ${ORIGIN}`))
